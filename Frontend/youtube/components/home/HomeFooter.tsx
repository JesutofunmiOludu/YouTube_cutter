  ...
  return (
    <footer className="border-t border-gray-300 py-8 px-6">
      <div className="max-w-content mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-2 text-body-sm font-medium text-gray-700">
        ...
        <div className="flex items-center gap-4">
          {['Privacy', 'Terms', 'Contact'].map((link) => (
            <Link key={link} href={`/${link.toLowerCase()}`}
              className="text-caption text-gray-600 hover:text-gray-800 transition-colors">
              {link}
            </Link>
          ))}
        </div>

...
export function HomeHowItWorks() {
  ...
  return (
    <section className="py-16 px-6 max-w-content mx-auto" aria-label="How it works">
      <h2 className="text-heading-xl text-[var(--color-text-primary)] text-center mb-10">How it works</h2>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {steps.map((step) => (
          <div key={step.step} className="flex flex-col items-center text-center">
            <div className="w-12 h-12 rounded-full flex items-center justify-center mb-4 bg-primary-50 border-2 border-primary-200 text-heading-lg text-primary-800 font-medium">
              {step.step}
            </div>
            <h3 className="text-heading-sm text-[var(--color-text-primary)] mb-2">{step.title}</h3>
            <p className="text-body-sm text-[var(--color-text-secondary)] leading-relaxed">{step.description}</p>
          </div>
        ))}
      </div>
    </section>
